        ...
        if self.use_sequence_packing:
            ...
            def loss_wrapper(output_tensor):
                ...
                for i in range(num_samples):
                    ...
                    loss_func_result = loss_func(single_data, single_output_tensor)
                    if len(loss_func_result) == 3:
                        loss, metrics, extra_tensors = loss_func_result
                        if (single_data.meta_info.get("need_collect_log_probs", False)
                                and "log_probs" in extra_tensors
                                and hasattr(self.worker, '_train_step_collected_log_probs')):
                            self.worker._train_step_collected_log_probs.append(extra_tensors["log_probs"].cpu())
                    else:
                        loss, metrics = loss_func_result
                    loss_result += loss
            ...
            def loss_wrapper(output_tensor):
                loss_func_result = loss_func(data, output_tensor)
                if len(loss_func_result) == 3:
                    loss, metrics, extra_tensors = loss_func_result
                    if (data.meta_info.get("need_collect_log_probs", False)
                            and "log_probs" in extra_tensors
                            and hasattr(self.worker, '_train_step_collected_log_probs')):
                        self.worker._train_step_collected_log_probs.append(extra_tensors["log_probs"].cpu())
                else:
...
class MegatronTrainStrategy(MegatronInferStrategy, TrainStrategy):
    ...
    def train_step(self, batch: DataProto, loss_func: Callable):
        ...
        # aicoder_ib parity: optionally collect training-time log_probs per micro-batch.
        need_collect_log_probs = batch.meta_info.get("need_collect_log_probs", False)
        if need_collect_log_probs and not hasattr(self.worker, '_train_step_collected_log_probs'):
        ...
        for micro_batch in micro_batches_list:
            micro_batch.meta_info['loss_scale'] = num_microbatches * mpu.get_data_parallel_world_size()
            micro_batch.meta_info['micro_batch_size'] = micro_batch.batch.batch_size[0]
            # Propagate collect flag into loss_wrapper (reads data.meta_info)
            micro_batch.meta_info['need_collect_log_probs'] = need_collect_log_probs
