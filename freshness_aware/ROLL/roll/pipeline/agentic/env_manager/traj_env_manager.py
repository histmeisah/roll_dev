...
class TrajEnvManager(BaseEnvManager):
    ...
    def formulate_rollouts(self, rollout_cache: RolloutCache):
        ...
        for key in ("model_answer", "gold_answer", "answer_source", "unboxed_answer"):
            if key in self.rollout_cache.history[-1]:
                lm_input.non_tensor_batch[key] = np.array(
                    [self.rollout_cache.history[-1][key]], dtype=object
                )
