...
class ActorWorker(Worker):
    ...
    def train_step(self, data: DataProto):
        ...
        with state_offload_manger(
            ...
            if self.worker_config.use_dynamic_batching_in_train:
                ...
                dataloader_kwargs = {"shuffle": not need_collect_log_probs}
        ...
        if need_collect_log_probs and hasattr(self, '_train_step_collected_log_probs'):
            if self._train_step_collected_log_probs:
                ...
                # Concatenate all collected log_probs. This path is only safe when
                # one log_probs row is returned per input row; otherwise the caller
                # should fall back to an explicit, ordered compute_log_probs pass.
                collected_log_probs = torch.cat(self._train_step_collected_log_probs, dim=0)
                expected_rows = data.batch.batch_size[0]
                if collected_log_probs.shape[0] != expected_rows:
                    self.logger.warning(
                        f"[DEBUG] Worker {self.worker_name} collected log_probs row mismatch: "
                        f"got={collected_log_probs.shape[0]}, expected={expected_rows}. "
                        "Dropping collected log_probs so caller can use fallback."
                    )
                    collected_log_probs = None
                else:
                    self.logger.debug(
    ...
    def loss_func(self, data: DataProto, output_tensor: torch.Tensor):
        ...
        # PER importance-sampling correction: the replay buffer writes per-sample weights
        # into data.batch["importance_weights"] when prioritized sampling is active. Multiplying
        # them into pg/kl loss is what cancels the over-weighting of high-priority samples.
        # None for on-policy fresh batches (no replay) → agg_loss falls back to unweighted.
        ...
        # aicoder_ib parity: 3-tuple return carries detached training-time log_probs
        # back to the strategy, which collects them per micro-batch and concats into
        # the returned DataProto so pipeline PHASE 10 can reuse them as behavior_log_probs
        # (saves one extra forward per step).
