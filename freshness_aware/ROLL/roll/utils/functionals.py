...
def compute_approx_kl(
    ...
    """
    # Defensive cast: when KL is disabled (init_kl_coef=0, use_kl_loss=false) or
    # log_probs come from zeros_like(attention_mask)/replay rehydration, the
    # tensors may arrive as bool/long. PyTorch >= 2.x rejects bool subtraction.
    if log_probs.dtype != torch.float32:
        log_probs = log_probs.float()
    if log_probs_base.dtype != torch.float32:
        log_probs_base = log_probs_base.float()
    if kl_penalty == "kl":
